import gen.implLexer;
import gen.implParser;
import gen.implVisitor;
...
class Interpreter extends AbstractParseTreeVisitor<Double> implements implVisitor<Double> {
	...
	public Double visitArrayAssignment(implParser.ArrayAssignmentContext ctx) {
    	...
    	int pos = visit(ctx.e1).intValue();

    	env.setVariable(ctx.x.getText() + '[' + pos + ']', v);
		return null;
	}
	...
	@Override
	public Double visitIfElseStatement(implParser.IfElseStatementContext ctx) {
	...
	public Double visitArrayVariable(implParser.ArrayVariableContext ctx) {
    	int pos = visit(ctx.e).intValue();
    	return env.getVariable(ctx.x.getText() + '[' + pos + ']');
	}
